...
def load_data(n):
    ...
    chars+=(EON,) #End of name
    chars+=(SON,) #Start of name
    vocab[SON] = len(chars)-2
    vocab[EON] = len(chars)-1
    
    length = np.array([len(n)+1 for n in names])
    names_input = [(SON+s).ljust(seq_length, EON) for s in names] 
    names_output = [s.ljust(seq_length, EON) for s in names] 
    names_input = np.array([np.array(list(map(vocab.get, s)))for s in names_input])
    names_output = np.array([np.array(list(map(vocab.get, s)))for s in names_output])
    prop = np.array([] for i in range(len(names)))
    return names_input, names_output, chars, vocab, prop, length 

# def load_data(n, seq_length=120):
#     import collections
#     f = open(n)
#     lines = f.read().split('\n')[:-1]
