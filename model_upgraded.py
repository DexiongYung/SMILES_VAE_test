...
class CVAE():
    ...
    def _create_network(self):
        ...
        self.weights['softmax'] = tf.compat.v1.get_variable("softmaxw", initializer=tf.random.uniform(
        ...
        self.weights['out_mean'] = tf.compat.v1.get_variable(
            "outmeanw", initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), shape=[self.unit_size, self.latent_size]),
        self.weights['out_log_sigma'] = tf.compat.v1.get_variable(
            "outlogsigmaw", initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform"), shape=[self.unit_size, self.latent_size]),
        self.biases['out_mean'] = tf.compat.v1.get_variable(
            "outmeanb", initializer=tf.compat.v1.zeros_initializer(), shape=[self.latent_size]),
        self.biases['out_log_sigma'] = tf.compat.v1.get_variable(
            "outlogsigmab", initializer=tf.compat.v1.zeros_initializer(), shape=[self.latent_size]),

        self.embedding_encode = tf.compat.v1.get_variable(name='encode_embedding', shape=[
                                                self.latent_size, self.vocab_size], initializer=tf.compat.v1.random_uniform_initializer(minval=-0.1, maxval=0.1))
        self.embedding_decode = tf.compat.v1.get_variable(name='decode_embedding', shape=[
                                                self.latent_size, self.vocab_size], initializer=tf.compat.v1.random_uniform_initializer(minval=-0.1, maxval=0.1))
        ...
        self.reconstr_loss = tf.reduce_mean(input_tensor=tf.contrib.seq2seq.sequence_loss(
    ...
    def encode(self):
        ...
        self.weights['out_log_sigma'] = tf.reshape(
            self.weights['out_log_sigma'], [self.unit_size, -1])
        mean = tf.matmul(h, self.weights['out_mean'])+self.biases['out_mean']
        log_sigma = tf.matmul(
            h, self.weights['out_log_sigma'])+self.biases['out_log_sigma']
    ...
    def decode(self, Z):
        ...
        self.initial_decoded_state = tuple([tf.nn.rnn_cell.LSTMStateTuple(tf.zeros(
        ...
        Y, self.output_decoded_state = tf.compat.v1.nn.dynamic_rnn(
            self.decoder, inputs, dtype=tf.float32, scope='decode', sequence_length=self.L, initial_state=self.initial_decoded_state)
    ...
    def cal_latent_loss(self, mean, log_sigma):
        latent_loss = tf.reduce_mean(input_tensor=-0.5*(1+log_sigma -
